...
class GravityRenderer(BaseSingleRenderer):
  ...
  def _remove_old_shots(self):
    # Remove old shots
    self.SHOTS = [shot for shot in self.SHOTS if shot['position'] > 0]
  ...
  def _render_shots(self):
    # Show all the shots
    for shot in self.SHOTS:
      self.SECTION.add_led(int(shot['position']), 1, shot['colour'])

  def do_render(self):
    self._simulate_shots()
    self._remove_old_shots()
    if len(
        self.SHOTS) < self.MAX_SHOTS and random.random() < self.SHOT_ADD_CHANCE:
      self._add_new_shot()
    self._render_shots()
