...
def get_rgb_rainbow(steps, max_rgb=127):
  ...
  HSV_tuples = [(x * 1.0 / steps, 1, 1) for x in range(steps)]
  RGB_tuples = [
      [hsv * max_rgb for hsv in colorsys.hsv_to_rgb(*hsv)] for hsv in HSV_tuples
  ]
...
def fade_in_out(rgb_colours, fade_steps_in, fade_steps_out, fade_hold_on,
                fade_hold_off):
  faded_colours = []
...
def make_palette(
  ...
  if colour is not None:
    ...
    palette = [resolve_colour(colour) for colour in colours]
  elif rainbow_steps is not None:
    palette = get_rgb_rainbow(rainbow_steps)
  else:
    raise Exception(
        'You must provide at least one of colour, colours or rainbow_steps!')

  # Apply fade if required
  if fade_steps_in:
    palette = fade_in_out(palette, fade_steps_in, fade_steps_out, fade_hold_on,
                          fade_hold_off)
